...
class TabaPapoController extends JControllerLegacy
{
    ...
    public function salasListar() {
	   ...
	   if (!JSession::checkToken('get')) 
        ...
        else 
        {
            ...
            $my_filter = $input->get('my_filter', 0, 'INT');
            ...
            $direction = $input->get('direction', 0, 'INT');
      		...
      		if ($page_actual)
      		{
      			$record = $model->listarSalas($page_actual, $my_filter, $list_limit, $filter, $search, $direction);
      			if ($record) 
      			{
   ...
   public function exitRoom() {
	...
	/* Method to integration with Community Builder Component. */

	public function cb_display () {

		$lang = JFactory::getLanguage();
		$lang->load('com_tabapapo', JPATH_SITE);
				
		$this->addViewPath(JPATH_SITE . '/components/com_tabapapo/views/');
		$this->addModelPath(JPATH_SITE . '/components/com_tabapapo/models/','tabapapoModel');
		
		$view = $this->getView('tabapapo', 'html');
		$view->addTemplatePath(JPATH_SITE . '/components/com_tabapapo/views/tabapapo/tmpl/');
		
		$model = $this->getModel('tabapapo', 'tabapapoModel');
		
		Form::addFormPath(JPATH_SITE . '/components/com_tabapapo/models/forms/');
