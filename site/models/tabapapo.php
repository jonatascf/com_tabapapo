...
class TabaPapoModelTabaPapo extends JModelList
{
	...
	public function getListTaba($my_filter, $filter, $search, $direction)
	{
		...
		if ($my_filter == 1)
		{
			$query->where('a.created_by = ' . $db->quote($usu_id));
		}
    ...
    public function listarSalas ($page_actual, $my_filter, $list_limit, $filter, $search, $direction) {
		
		$currentuser = JFactory::getuser();
		$usu_id = $currentuser->get("id");
			
		$delusers = $this->deleteUsers(null, 'no-msg');
		...
		$db->setQuery($this->getListTaba($my_filter, $filter, $search, $direction));
		$db->execute();

		$rows[0]= $db->getNumRows();

		$db->setQuery($this->getListTaba($my_filter, $filter, $search, $direction), $start_limit, $list_limit);
  ...
  public function nSalas () {
	...
	public function enviarMensagem($form) {
		...
		if ($talkto_id != 0) {
			...
			$queryusu = $db->getQuery(true);

			$queryusu->select($db->quoteName(array('id','usu_id')));
			$queryusu->from($db->quoteName('#__tabapapo_usu'));
			$queryusu->where($db->quoteName('usu_id').'='.$db->quote($talkto_id));
			$queryusu->where($db->quoteName('sala_id').'='.$db->quote($sala_id));

			$db->setQuery($queryusu);
			$db->execute();
			$num_rows = $db->getNumRows();
			$results = $db->loadObjectlist();		
			...
			if ($num_rows == null){
				...
				$msg_error = ['msg' => Text::_('COM_TABAPAPO_USEROFF')];
				return $msg_error;
	...
	public function insert_user($sala_id) {
      ...
      if ($currentuser->get("id") > 0) {
		...
		try {  
			...
			$params = new stdClass();
			$params->usu_name = $currentuser->get("username");
			$params->getin_time = $result[0]->now;
			$params = json_encode($params);
